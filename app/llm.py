...
def _mock_classify(query: str) -> dict[str, Any]:
    ...
    _stop = (
        ...
        r"план\w*|отлич\w*|"
        # слова о форме ответа, а не о показателе: «структура сделок», «топ 3 источника»
        r"структур\w*|состав\w*|разбивк\w*|детализ\w*|динамик\w*|топ|источник\w*|канал\w*|"
        r"лучш\w*|худш\w*|всего|итого|что будет|ожидат\w*|сильнее|слабее|"
    ...
    if re.search(r"прогноз|предскаж|ожидат|перспектив|что будет|сколько будет", q):
        intent = "forecast"
    elif re.search(
        ...
        r"сам\w+\s+(слаб|сильн|крупн|больш|мал|лучш|худш)|лидер|аутсайдер|худш|лучш|"
        r"кто упал|что упало|какие .+ (растут|падают)", q,
    ) and not re.search(r"\bили\b|\bvs\b", q):  # «что лучше X или Y» — сравнение двух
        intent = "rank"
    elif re.search(
        r"по сравнению|сравни|изменил|во сколько|на ?сколько|отлич\w*|рост|снижени|дол[яи] .+ (в|от) |"
        r"что лучше|лучше .+ или|упал|вырос|растёт|растет|падает|снизил|динамик|тренд|как измен", q,
    ):
        intent = "compare"
